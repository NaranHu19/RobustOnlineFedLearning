...
def loss_heatmap(path_to_results: str, path_to_plot: str) -> None:
    """
    Create a heatmap of training losses.

    Use the number of Byzantine clients and the data-distribution
    parameter as the heatmap axes. Each cell contains the mean of the
    best training losses reached across seeds for a given aggregation.

    Parameters
    ----------
    path_to_results : str
        Path containing benchmark results.
    path_to_plot : str
        Directory in which the heatmap is stored.
    """
    try:
        with open(os.path.join(path_to_results, "config.json")) as file:
            data = json.load(file)
                    ...
                    for am in am_list:
                        for nb_client in nb_clients:
                            for nb_decl in nb_declared:
                                actual_nb_byz = [
                                    item for item in nb_byz if item <= nb_decl
                                ]
                                heat_map_table = np.zeros(
                                    (
                                        len(distribution_parameter_list),
                                        len(actual_nb_byz),
                                    )
                                )
                                ...
                                for y, nb_byzantine in enumerate(actual_nb_byz):
                                    ...
                                    for x, dist_param in enumerate(
                                        ...
                                        for attack in attacks:
                                            ...
                                            nb_steps = data["benchmark_config"][
                                                "nb_steps"
                                            ]
                                            ...
                                            for run_dd in range(
                                                ...
                                                for run in range(nb_training_seeds):
                                                    losses[run_dd][run] = genfromtxt(
                                                        f"{path_to_results}/"
                                                        f"{config_file_name}/"
                                                        f"train_loss_tr_seed_"
                                                        f"{run + training_seed}_"
                                                        f"dd_seed_"
                                                        f"{run_dd + data_distrib_seed}"
                                                        f".txt",
                                                        delimiter=",",
                                        ...
                                        heat_map_table[len(heat_map_table) - 1 - x][
                                            y
                                        ] = lowest_loss
                                ...
                                file_name = (
                                    ...
                                    f"{pre_agg_names}_"
                                    f"{agg['name']}_"
                                    f"nb_clients_{nb_client}_"
                                    f"aggreg_freq_scale_{af}_"
                                    f"aggreg_mult_scale_{am}_" + end_file_name
                                )

                                column_names = [
                                    str(dist_param)
                                    for dist_param in distribution_parameter_list
                                ]
                                row_names = [
                                    str(nb_byzantine) for nb_byzantine in actual_nb_byz
                                ]
...
def test_heatmap(path_to_results: str, path_to_plot: str) -> None:
    """
    Create a heatmap of test accuracies.

    Use the number of Byzantine clients and the data-distribution
    parameter as the heatmap axes. Each cell contains the mean of the
    best accuracy reached across seeds for a given aggregation.
                                ...
                                for y, nb_byzantine in enumerate(actual_nb_byz):
                                    ...
                                    for x, dist_param in enumerate(
                                        ...
                                        for attack in attacks:
                                            ...
                                            nb_accuracies = int(
                                                1
                                                + math.ceil(nb_steps / evaluation_delta)
                                            )
...
def aggregated_test_heatmap(
    path_to_results: str,
    path_to_plot: str,
) -> None:
    """
    Create an aggregated test-accuracy heatmap.

    Compare all aggregators and display the best accuracy obtained in
    each region of the heatmap.
    ...
    path_to_results : str
        Path containing benchmark results.
    path_to_plot : str
        Directory in which the aggregated heatmap is stored.
    """
    try:
        with open(os.path.join(path_to_results, "config.json")) as file:
            data = json.load(file)
            ...
            for am in am_list:
                for nb_client in nb_clients:
                    for nb_decl in nb_declared:
                        ...
                        for data_dist in data_distributions:
                            data_dist["distribution_parameter"] = ensure_list(
                                data_dist["distribution_parameter"]
                            )
                            distribution_parameter_list = data_dist[
                                "distribution_parameter"
                            ]
                            heat_map_cube = np.zeros(
                                (
                                    len(aggregators),
                                    len(distribution_parameter_list),
                                    len(actual_nb_byz),
